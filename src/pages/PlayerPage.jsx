...
function BuzzerButton({ gameState, teamId, publishAction }) {
    ...
    let buttonText = t('buzzer_push'); 
    if (!timerRunning) buttonText = t('buzzer_wait'); 
    if (hasBuzzed) buttonText = t('buzzer_done');
    return (
...
function PlayerPage() {
  ...
  if (!scannedData && !gameId) {
    return (
      <div>
        ...
        {showScanner && (
            <div>
                <button onClick={() => setShowScanner(false)}>{t('cancel')}</button>
        ...
        {showMyQrMode && (
            <div style={{ textAlign: 'center', border: '2px dashed #ccc', padding: '20px', margin: '20px' }}>
                ...
                <br />
                <button onClick={() => setShowMyQrMode(false)} style={{ marginTop: '20px' }}>{t('cancel')}</button>
            </div>
        )}
      </div>
    );
  ...
  if (!isConnected || !gameState) return <div style={{padding:'20px'}}><h2>{t('connecting')}</h2></div>;

  // Стан 2: Вибір команди (якщо gameId є, а teamId немає)
  if (scannedData && !scannedData.teamId) {
      return (
          <div style={{ padding: '20px', textAlign: 'center' }}>
              <button onClick={handleExitGame} style={{ float: 'right', fontSize: '0.8em', background: '#8a3c3cff' }}>{t('exit')}</button>
              <h1>{t('youAreInTheGame', { gameId })}</h1>
              <h2>{t('selectYourTeam')}</h2>
              
              {teams.length === 0 ? (
                  <p>{t('hostHasNotCreatedTeams')}</p>
              ) : (
              ...
              <p>{t('orScanTeamQR')}</p>
              <button onClick={() => { setScannedData(null); setShowScanner(true); }}>{t('scanQR')}</button>
          </div>
      )
  }
  ...
  return (
    <div style={{padding: '20px', minHeight: '100vh', textAlign: 'center' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
         <h3>{myTeam.name}</h3>
         <button onClick={handleExitGame} style={{ fontSize: '0.8em', background:'#973030ff', border:'1px solid red', padding: '5px' }}>{t('exit')}</button>
      </div>

      <div style={{ minHeight: '60px', margin: '10px 0', padding: '10px', border: '1px solid #ccc', borderRadius: '8px', background: '#1a1a1a' }}>
          {gameState.timerRunning && currentQuestionText ? <h3>{currentQuestionText}</h3> : <p style={{color:'#888'}}>{t('waitForQuestion')}</p>}
      </div>
      ...
      <table style={{ width: '100%', maxWidth: '800px', margin: '0 auto', borderCollapse: 'collapse' }}>
        <thead><tr style={{ backgroundColor: '#707070ff' }}><th>{t('team')}</th><th>{t('score')}</th><th>{t('time')}</th></tr></thead>
        <tbody>
          {teams.map((team) => {
            const buzzData = buzzers.find(b => b.teamId === team.id);
            return (
              <tr key={team.id} style={{ fontWeight: team.id === scannedData.teamId ? 'bold' : 'normal' }}>
                <td style={{ maxWidth: '50vw', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', border: '1px solid #ccc' }}>{team.name}</td>
      ...
      <div style={{ marginTop: '20px' }}><button onClick={() => setScannedData({ ...scannedData, teamId: null })} style={{ display: 'none', fontSize: '0.8em', color: '#555' }}>{t('changeTeam')}</button></div>
    </div>
  );
}
